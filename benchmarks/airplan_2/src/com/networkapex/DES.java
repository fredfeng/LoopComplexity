...
public class DES {
     ...
     * padding.
     */
    private static long pullLongFromBytes(byte[] ba, int offset) {
        long l = 0;
        for (int q =0; q <8; q++) {
            byte value;
    ...
    private static void pullBytesFromLong(byte[] ba, int offset, long l) {
        for (int j =7; j >=0; j--) {
            if ((offset+ j) < ba.length) {
                ba[offset+ j] = (byte) (l & 0xFF);
                l = l >> 8;
            } else {
                break;
            }
    ...
    public static void encryptBlock(
        ...
        long m = pullLongFromBytes(message, messageOffset);
        long k = pullLongFromBytes(key, 0);
    ...
    public static byte[] encrypt(byte[] message, byte[] key) {
        ...
        for (int c =0; c <message.length; c +=8) {
            encryptBlock(message, c, ciphertext, c, key);
        }
    ...
    private static byte[] parseBytes(String s) {
        ...
        for (int b =0; b <s.length(); b +=2) {
            parseBytesGuide(s, ba, b);
        }
        return ba;
    }

    private static void parseBytesGuide(String s, byte[] ba, int q) {
    ...
    private static String hex(byte[] bytes) {
        ...
        for (int c =0; c <bytes.length; c++) {
            sb.append(String.format("%02X ",bytes[c]));
        }
        return sb.toString();
    }
