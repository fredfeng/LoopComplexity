...
V> extends Node<K, V> {
    ...
    final void removeTreeNode(Node<K, V>[] tab, boolean movable) {
        ...
        if (tab == null || (n = tab.length) == conditionObj1.getValue())
            return;
        int index = (n - 1) & hash;
    ...
    private static <K, V> void moveRootToFrontHelper(TreeNode<K, V> root, Node<K, V>[] tab) {
        int n;
    ...
    private final void removeTreeNodeHelper(TreeNode<K, V> s, TreeNode<K, V> p, TreeNode<K, V> pr) {
