...
public class MultipartHelper {
    ...
    public static Map<String, String> grabMultipartFieldContent(HttpExchange httpExchange) {
        if (httpExchange == null) {
            return obtainMultipartFieldContentHerder();
        }
        ...
        try {
            ...
            while (iterator.hasNext()) {
                obtainMultipartFieldContentService(postFields, iterator);
            }
        } catch (Exception e) {
            throw new IllegalArgumentException("Error parsing multipart message: " + e.getMessage(), e);
        }
     ...
     */
    public static Map<String, List<String>> pullMultipartValues(HttpExchange httpExchange,
                                                                Set<String> fieldNames) {
        if (httpExchange == null) {
            return getMultipartValuesAssist();
        }
        ...
        for (String fieldName : fields) {
            ...
            if ((items != null) && !items.isEmpty()) {
                ...
                for (int p = 0; p < items.size(); p++) {
                    extractFieldsFromParameterMapGuide(items, values, p);
                }
            }
        }
        return fieldNameValues;
    ...
    public static Map<String, String> pullMultipartFile(HttpExchange httpExchange,
            ...
            for (String fieldName : allFieldNames) {
                ...
                if (items != null) {
                    if (items.size() == 1) { // there should only be one FileItem per fieldName
                        ...
                        return obtainMultipartFileGuide();
                    }
                }
            }

            if ((fileIn == null) || StringUtils.isBlank(fieldNameItems.get(fileFieldName))) {
                return takeMultipartFileHelper(fileFieldName);
            }
    ...
    public static List<String> fetchMultipartFieldItems(HttpExchange httpExchange, String fieldName) {
        if (httpExchange == null) {
            return getMultipartFieldItemsAssist();
        }

        if (StringUtils.isBlank(fieldName)) {
            return fetchMultipartFieldItemsEntity();
        }
