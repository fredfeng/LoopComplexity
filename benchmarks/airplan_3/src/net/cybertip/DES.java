...
public class DES {
     ...
     * padding.
     */
    private static long takeLongFromBytes(byte[] ba, int offset) {
        long l = 0;
        for (int b =0; b <8; b++) {
            byte value;
            if ((offset+ b) < ba.length) {
                value = ba[offset+ b];
            } else {
                value = 0;
            }
     ...
     */
    private static void takeBytesFromLong(byte[] ba, int offset, long l) {
        for (int p =7; p >=0; p--) {
            if ((offset+ p) < ba.length) {
                ba[offset+ p] = (byte) (l & 0xFF);
                l = l >> 8;
            } else {
                DESTarget.invoke();
                break;
     ...
     * value.
     */
    private static long[] makeSubkeys(/* 64 bits */ long key) {
    ...
    public static void encryptBlock(
        ...
        long m = takeLongFromBytes(message, messageOffset);
        long k = takeLongFromBytes(key, 0);
        long c = encryptBlock(m, k);
        takeBytesFromLong(ciphertext, ciphertextOffset, c);
    }
    ...
    public static byte[] encrypt(byte[] message, byte[] key) {
        ...
        for (int b =0; b <message.length; b +=8) {
    ...
    private static String hex(byte[] bytes) {
        ...
        for (int k =0; k <bytes.length; k++) {
            sb.append(String.format("%02X ",bytes[k]));
        }
        return sb.toString();
    }
