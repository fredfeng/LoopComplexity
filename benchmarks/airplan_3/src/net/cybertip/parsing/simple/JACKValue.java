...
public class JACKValue {
	 ...
	 * 
	 * @see JACKExtractor#parse(Reader)
	...
	public static void writeJACKString(Object value, Writer out) throws IOException {
		...
		if((value instanceof JACKAware)){
            writeJACKStringTarget((JACKAware) value, out);
            return;
		}
		
		if(value instanceof Map){
            writeJACKStringSupervisor((Map) value, out);
            return;
		}
		
		if(value instanceof List){
            writeJACKStringHerder((List) value, out);
            return;
		}
