...
public class TemplateEngine {
    ...
    public List<Pair<Integer, Integer>> encounterTags() {
        ...
        while (matcher.find()) {
            encounterTagsWorker(matcher, tagsList);
        }
        return tagsList;
    }
    ...
    public void replaceTagsBuilder(Map<String, String> dictionary, StringBuilder sb) {
        ...
        for (int b = 0; b < tagsList.size(); b++) {
            ...
            int startTagLocation = tagsList.get(b).getLeft();
            int endTagLocation = tagsList.get(b).getRight();
    ...
    public String replaceTags(Templated templated) {
        return replaceTags(templated.takeTemplateMap());
    }
    ...
    public String replaceTags(List<? extends Templated> templateds, String separator) {
        StringBuilder sb = new StringBuilder();
        for (int p = 0; p < templateds.size(); p++) {
            Templated templated = templateds.get(p);
            replaceTagsBuilder(templated.takeTemplateMap(), sb);
            sb.append(separator);
        }
        return sb.toString();
    }
