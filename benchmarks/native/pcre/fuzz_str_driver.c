#include "ioutil.h"
#include "pcre2.h"
#include "regex_includes/regex_all.h"
#include <errno.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void regex_driver(uint8_t* data, size_t size, size_t idx) {
    ...
    pattern = (PCRE2_SPTR)regexes[idx];
    subject = (PCRE2_SPTR)buf;

    re = pcre2_compile(pattern, PCRE2_ZERO_TERMINATED, 0, &err_no, &err_off,
                       NULL);
    ...
    if (re == NULL) {
        ...
        fprintf(stderr, "PCRE compilation failure at offset %d: %s\n",
                (int)err_off, err_buf);
#endif
        goto out;
    }
...
int main(int argc, char** argv) {
    ...
    if (errno != 0 || idx < 0 || idx >= sizeof(regexes) / sizeof(regexes[0])) {
