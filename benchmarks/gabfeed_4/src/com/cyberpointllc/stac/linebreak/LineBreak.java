...
public class LineBreak {
    ...
    public List<String> breakLines(String text) {
        ...
        for (int i = 1; i < offsets.length; ) {
            ...
            for (; i < offsets.length && randomNumberGeneratorInstance.nextDouble() < 0.5; i++) {
                breakLinesHelper(words, offsets, i);
            }
        }
    ...
    private void search(int i0, int j0, int i1, int j1, long[] minima, int[] breaks, int[] offsets) {
        searchHelper(i0, offsets, breaks, j0, j1, minima, i1);
    }
    ...
    private void breakLinesHelper(String[] words, int[] offsets, int i) {
        offsets[i] = offsets[i - 1] + words[i - 1].length();
    }

    private void searchHelper(int i0, int[] offsets, int[] breaks, int j0, int j1, long[] minima, int i1) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (currentJ0 < currentJ1) {
                ...
                for (int i = currentI0; i < currentI1; ) {
                    Random randomNumberGeneratorInstance = new  Random();
                    for (; i < currentI1 && randomNumberGeneratorInstance.nextDouble() < 0.5; i++) {
