...
public class ThreadHandler extends GabHandler {
    ...
    private String getContents(GabThread thread, WebSession webSession) {
        ...
        for (GabMessage message : messages) {
            getContentsHelper(suppressTimestamp, message, webSession, builder);
        }
    ...
    // Jia: added this method for convenience
    public static String getContentsPublic(GabThread thread) {
        StringBuilder builder = new  StringBuilder();
        List<GabMessage> messages = thread.getMessages();
        Sorter sorter = new  Sorter(GabMessage.ASCENDING_COMPARATOR);
        messages = sorter.sort(messages);

        WebTemplate messageListTemplate = new WebTemplate
                ("MessageListSnippet.html", ThreadHandler.class);
        for (GabMessage message : messages) {
            getContentsHelperStatic(message, messageListTemplate, builder);
        }

        String messageContents = builder.toString();
        Map<String, String> threadMap = thread.getTemplateMap();
        threadMap.put("messages", messageContents);
        WebTemplate threadTemplate = new  WebTemplate("ThreadTemplate.html",
                ThreadHandler.class);
        return threadTemplate.getEngine().replaceTags(threadMap);
    }

    // Jia: added this method for convenience
    private static void getContentsHelperStatic(GabMessage message,
                                                WebTemplate template,
                                                StringBuilder builder) {
        Map<String, String> messageMap = message.getTemplateMap();
        // fix up the contents
        String content = messageMap.get("messageContents");
        messageMap.put("messageContents", PageUtils
                .formatLongStringNoSesstion(content));
        template.getEngine().replaceTagsBuilder(messageMap, builder);
    }

    private void getContentsHelper(boolean suppressTimestamp, GabMessage message, WebSession webSession, StringBuilder builder) {
        Map<String, String> messageMap = message.getTemplateMap();
        // fix up the contents
        String content = messageMap.get("messageContents");
