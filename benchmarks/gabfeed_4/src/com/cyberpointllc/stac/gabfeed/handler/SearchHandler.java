...
public class SearchHandler extends GabHandler {
    ...
    private String getTermsText() {
        ...
        int i;
        for (i = 0; i < terms.size() - 1; ) {
            Random randomNumberGeneratorInstance = new  Random();
            for (; i < terms.size() - 1 && randomNumberGeneratorInstance.nextDouble() < 0.5; i++) {
                text += terms.get(i) + ", ";
            }
        }
            ...
            while ((line = reader.readLine()) != null) {
                if (line.contains("=")) {
                    if (key.length() != conditionObj0.getValue()) {
                        getInfoTextHelper(val, textInfo, key);
                    }
    ...
    private void getInfoTextHelper(String val, Map<String, String> textInfo, String key) throws IOException {
        textInfo.put(key, val);
    }
}
