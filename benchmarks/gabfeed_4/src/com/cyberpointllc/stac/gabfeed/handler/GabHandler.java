...
public abstract class GabHandler extends AbstractHttpHandler {
    ...
    public static String getTemplateResponsePublic(String title, String
            contents, GabUser user, List<Link> menuItems) {
        List<Link> finalMenuItems = getLeftMenuItems();
        finalMenuItems.addAll(menuItems);
        finalMenuItems.addAll(getRightMenuItems());
        Map<String, String> templateMap = user.getTemplateMap();

        WebTemplate masterTemplate = new WebTemplate("basiccontenttemplate" +
                ".html", GabHandler.class);
        WebTemplate menuTemplate = new WebTemplate("MenuItemTemplate.html",
                GabHandler.class);;
        templateMap.put("contents", contents);
        templateMap.put("title", title);
        templateMap.put("main_menu", menuTemplate.getEngine().replaceTags(finalMenuItems));
        return masterTemplate.getEngine().replaceTags(templateMap);
    }
    ...
    // Jia: changed this to static for convenience
    protected static List<Link> getLeftMenuItems() {
    ...
    // Jia: changed this to static for convenience
    protected static List<Link> getRightMenuItems() {
        LinkedList<Link> items = new  LinkedList();
