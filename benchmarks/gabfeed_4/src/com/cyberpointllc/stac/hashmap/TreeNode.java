...
K, V> extends Node<K, V> {
    ...
    static <K, V> void moveRootToFront(Node<K, V>[] tab, TreeNode<K, V> root) {
        ...
        if (root != null && tab != null && (n = tab.length) > conditionObj0.getValue()) {
            ...
            if (root != first) {
                moveRootToFrontHelper(index, root, tab, first);
            }
    ...
    private static <K, V> void moveRootToFrontHelper(int index, TreeNode<K, V> root, Node<K, V>[] tab, TreeNode<K, V> first) {
        Node<K, V> rn;
        tab[index] = root;
    ...
    private final void splitHelper(int bit, int index, Node<K, V>[] tab) {
        ...
        for (TreeNode<K, V> e = b, next; e != null; ) {
            Random randomNumberGeneratorInstance = new  Random();
            for (; e != null && randomNumberGeneratorInstance.nextDouble() < 0.5; e = next) {
